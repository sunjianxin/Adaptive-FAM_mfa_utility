...
struct Block : public BlockBase<T>
{
	...
	T face(VectorX<T>&  domain_pt,
           DomainArgs&  args,
           int          k)                  // current science variable
    {
        DomainArgs* a = &args;
        T retval = 1.0;
        for (auto i = 0; i < this->dom_dim; i++) // 2
            retval *= sin(domain_pt(i) * a->f[k]);
        retval *= a->s[k];
		
		// std::cout << "-------" << domain_pt(0) << ", " << domain_pt(1) << std::endl;
...
//         T alpha = 0.25;
           ...
           DomainArgs&  args)
    {
        ...
        T& z            = domain_pt(2);
        T retval;
        // if (x <= 0.5 && x >= -0.5 && y <= 0.5 && y >= -0.5 && z <= 0.5 && z >= -0.5 && z <= -x + 0.5) {
        if (x <= 0.5 && x >= 0.0 && y <= 0.5 && y >= 0.0 && z <= 0.5 && z >= 0.0 && z <= -x + 0.5) {
            retval = 0.5;
        } else {
            retval = 0.0;
        }
            ...
            DomainArgs&                         args)
    {
		...
		int face_data_size = 256*256;
		float face_data[face_data_size];
		std::ifstream infile;
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_0.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_1.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_2.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_3.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_4.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_5.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_6.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_7.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_8.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_9.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_10.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_11.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_12.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_13.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_14.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_15.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_16.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_17.txt");
    	// infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_18.txt");
    	infile.open("/home/js/ws/pacificVis/repurposingGANs/repurpose-gan/repurpose-gan/training_data/image_data_19.txt");
    	assert(infile);
    	std::string line;
    	int ctr = 0;
		...
		for (int j = 0; j < input->domain.rows(); j++) // rows = sample size ^ 3
        {
            ...
            // std::cout << "j: " << j << ", xyz: " << dom_pt(0) << " " << dom_pt(1) << " " << dom_pt(2) << std::endl; 
            T retval;
            // if (j%skip == 0) {
            //     std::cout << j << std::endl;
            // }
            for (auto k = 0; k < nvars; k++)        // for all science variables, is 1 normally
            {
                if (fun == "sine")
                    retval = sine(dom_pt, args, k);
				if (fun == "face")
                    // retval = face(dom_pt, args, k);
                    retval = face_data[j];
                if (fun == "sinc")
            ...
            if (minn > input->domain(j, this->dom_dim)) { minn = input->domain(j, this->dom_dim); }
            if (maxx < input->domain(j, this->dom_dim)) { maxx = input->domain(j, this->dom_dim); }
...
//         cerr << "domain:\n" << this->domain << endl;
    ...
    // Read in structured discrete general dataset into corresponding block partitioned, only 8 blocks are allowed for it was hardcoded
    void read_3d_scalar_data_multiblock_8(
            const diy::Master::ProxyWithLink&   cp,
            DomainArgs&                         args)
    {
        DomainArgs* a   = &args;
        ...
        //       Not always true, can model a 2d surface in 3d, e.g.
        //       Also assumes each var is scalar
        int nvars       = this->pt_dim - this->dom_dim;             // number of science variables, is 1 for scalar field
        ...
        std::cout << "pt_dim: " << pt_dim << std::endl;
        std::cout << "ndom_pts.prod(): " << ndom_pts.prod() << std::endl;
        std::cout << "ndom_pts: " << ndom_pts << std::endl;
        std::cout << "infile: " << a->infile << std::endl;
        ...
        // TODO: hard coded to 8 blocks, need to change to any number of blocks
        int partition_dimension = 2; // hardcoded for 8 partitions
        int block_x = cp.gid()%(partition_dimension*partition_dimension)%partition_dimension;
        int block_y = cp.gid()%(partition_dimension*partition_dimension)/partition_dimension;
        int block_z = cp.gid()/(partition_dimension*partition_dimension);
        int global_size_x = partition_dimension*(ndom_pts(0) - 1) + 1;
        int global_size_y = partition_dimension*(ndom_pts(1) - 1) + 1;
        int global_size_z = partition_dimension*(ndom_pts(2) - 1) + 1;
        int total_samples = global_size_x*global_size_y*global_size_z;
        std::cout << "block xyz: " << block_x << " " << block_y << " " << block_z << std::endl;
        std::cout << "global size xyz: " << global_size_x << " " << global_size_y << " " << global_size_z << std::endl;

        vector<float> val(total_samples);
        FILE *fd = fopen(a->infile.c_str(), "rb");
        assert(fd);
        if (!fread(&val[0], sizeof(float), total_samples, fd))
        {
            fprintf(stderr, "Error: unable to read file\n");
            exit(0);
        }
        ...
        for (int j = 0; j < input->domain.rows(); j++) // rows = sample size ^ 3
        {
            dom_pt = input->domain.block(j, 0, 1, this->dom_dim).transpose();
            // std::cout << "j: " << j << ", xyz: " << dom_pt(0) << " " << dom_pt(1) << " " << dom_pt(2) << std::endl;
            // translate from local coordinate to global coordinate
            int local_x = j%(ndom_pts(0)*ndom_pts(1))%ndom_pts(0);
            int local_y = j%(ndom_pts(0)*ndom_pts(1))/ndom_pts(0);
            int local_z = j/(ndom_pts(0)*ndom_pts(1));

            
            int global_x = block_x*(ndom_pts(0) - 1) + local_x;
            int global_y = block_y*(ndom_pts(1) - 1) + local_y;
            int global_z = block_z*(ndom_pts(2) - 1) + local_z;

            int index = global_z*global_size_x*global_size_y + global_y*global_size_x + global_x;

            T retval;
            
            input->domain(j, 3) = val.at(index);
...
//         cerr << "domain:\n" << this->domain << endl;
    ...
    // Read in structured discrete general dataset into corresponding block partitioned
    void read_3d_scalar_data_multiblock_512(
            const diy::Master::ProxyWithLink&   cp,
            DomainArgs&                         args)
    {
        DomainArgs* a   = &args;
        ...
        std::cout << "%%%%%%%%%%%%%%%%%%%%%" << std::endl;
        std::cout << "pid: " << cp.gid() << std::endl;
        std::cout << "dom_dim: " << dom_dim << std::endl;
        std::cout << "pt_dim: " << pt_dim << std::endl;
        std::cout << "ndom_pts.prod(): " << ndom_pts.prod() << std::endl;
        std::cout << "ndom_pts: " << ndom_pts << std::endl;
	std::string local_file_name = a->infile + "/" + std::to_string(cp.gid()) + ".xyz";
        std::cout << "will read file: " << local_file_name << std::endl;
        ...
        // TODO: hard coded to 512 blocks, need to change to any number of blocks
        int partition_dimension = 8; // hardcoded for 512 partitions
        int total_samples = ndom_pts(0)*ndom_pts(1)*ndom_pts(2);
        std::cout << "total_samples: " << total_samples << std::endl;

        vector<float> val(total_samples);
        FILE *fd = fopen(local_file_name.c_str(), "rb");
        assert(fd);
        if (!fread(&val[0], sizeof(float), total_samples, fd))
        {
            fprintf(stderr, "Error: unable to read file\n");
...
//         cerr << "domain:\n" << this->domain << endl;
    ...
    // Read in structured discrete general dataset into corresponding block partitioned
    void read_3d_scalar_data_multiblock_all(
            const diy::Master::ProxyWithLink&   cp,
            DomainArgs&                         args)
    {
        DomainArgs* a   = &args;
...
//                      vel[3 * i], vel[3 * i + 1], vel[3 * i + 2]);
            ...
            DomainArgs& args)
    {
        ...
        // find extent of range
        std::cout << "domain.rows(): " << input->domain.rows() << std::endl;
        std::cout << "bounds_mins: " << bounds_mins << std::endl;
        std::cout << "bounds_maxs: " << bounds_maxs << std::endl;
        for (size_t i = 0; i < (size_t)input->domain.rows(); i++)
        {
            if (i == 0 || input->domain(i, 3) < bounds_mins(3))
                bounds_mins(3) = input->domain(i, 3);
        ...
        std::cout << "ndom_pts: " << ndom_pts << std::endl;
        int n = 0;
        for (size_t k = 0; k < (size_t)(ndom_pts(2)); k++) // 64
            for (size_t j = 0; j < (size_t)(ndom_pts(1)); j++) // 64
                for (size_t i = 0; i < (size_t)(ndom_pts(0)); i++) // 64
                {
                    input->domain(n, 0) = i;
                    input->domain(n, 1) = j;
                    input->domain(n, 2) = k;
