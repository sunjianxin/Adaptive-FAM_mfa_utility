...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < pt_dim - dom_dim; i++) // dom_dim = 3
        d_args.f[i] = 1.0;
    for (int i = 0; i < dom_dim; i++) // dom_dim = 3
    {
        d_args.geom_p[i]            = geom_degree; // 1
        d_args.vars_p[0][i]         = vars_degree;      // assuming one science variable, vars_p[0], -q
        d_args.ndom_pts[i]          = ndomp; // -m
        d_args.geom_nctrl_pts[i]    = geom_nctrl; // -1, -g
        d_args.vars_nctrl_pts[0][i] = vars_nctrl; // -1, -v      // assuming one science variable, vars_nctrl_pts[0]
    }
...
//         d_args.infile       = "/Users/tpeterka/datasets/rti/dd07g_xxsmall_le.xyz";
    ...
    if (input == "general")
    {
        ...
        d_args.vars_nctrl_pts[0][0] = vars_nctrl;
        d_args.vars_nctrl_pts[0][1] = vars_nctrl;
        d_args.vars_nctrl_pts[0][2] = vars_nctrl;
