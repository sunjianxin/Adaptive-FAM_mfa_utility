...
int main(int argc, char** argv)
{
	...
	// get face 2d data for testing gan based paraemeter learning idea
	if (input == "face")
    {
        for (int i = 0; i < dom_dim; i++)
        {
		...
		std::cout << "dom_dim: " << dom_dim << std::endl; // 2 
		std::cout << "pt_dim: " << pt_dim << std::endl; // 3
        for (int i = 0; i < pt_dim - dom_dim; i++)      // for all science variables
            d_args.s[i] = i + 1;                        // scaling factor on range
        master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
                { b->generate_analytical_data(cp, input, d_args); });
...
//      d_args.infile = "/Users/tpeterka/datasets/CESM-ATM-tylor/1800x3600/FLDSC_1_1800_3600.dat";
    ...
    if (input == "general")
    {
        ...
        if (dom_dim == 3)
            master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
                    { b->read_3d_scalar_data(cp, d_args); });
        else
        {
    ...
    if (input == "tooth")
    {
        d_args.ndom_pts.resize(3);
        d_args.vars_nctrl_pts[0].resize(3);
        d_args.ndom_pts[0]  = 103;
        d_args.ndom_pts[1]  = 94;
        d_args.ndom_pts[2]  = 161;
        d_args.vars_nctrl_pts[0][0] = 51;
        d_args.vars_nctrl_pts[0][1] = 47;
        d_args.vars_nctrl_pts[0][2] = 80;
        d_args.infile = infile;
        if (dom_dim == 3)
            master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
                    { b->read_3d_scalar_data(cp, d_args); });
        ...
        {
            fprintf(stderr, "bonsai data only available in 3d domain\n");
            exit(0);
        }
    }
    // compute the MFA
