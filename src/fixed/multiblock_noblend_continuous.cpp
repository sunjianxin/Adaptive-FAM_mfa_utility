...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < dom_dim; i++) // dom_dim = 3, for x, y, z dimention
    {
        ...
        if (strong_sc)                          // strong scaling, reduced number of points per block
        {
            ...
            d_args.vars_nctrl_pts[0][i] = vars_nctrl / divs[i];     // divs[i] = partition size on each x, y, z direction. assuming one science variable, vars_nctrl_pts[0]
            std::cout << "num of contropoints for each partition's x, y, z dimension, d_args.vars_nctrl_pts[0][i]: " << d_args.vars_nctrl_pts[0][i] << std::endl;  
    ...
    if (input == "general")
    {
        ...
        master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
                // { b->read_3d_scalar_data_multiblock_8(cp, d_args); });
                // { b->read_3d_scalar_data_multiblock_512(cp, d_args); });
                { b->read_3d_scalar_data_multiblock_all(cp, d_args); });
    }
    ...
    if (input == "general")
    {
        ...
        std::cout << "ndomp: " << ndomp << std::endl;
        std::cout << "vars_nctrl: " << vars_nctrl << std::endl;
        d_args.ndom_pts[0]  = ndomp; // -n, was 256;
        d_args.ndom_pts[1]  = ndomp; // -n, was 256;
        d_args.ndom_pts[2]  = ndomp; // -n, was 256;
        d_args.vars_nctrl_pts[0][0] = vars_nctrl; // -v, was 128
        d_args.vars_nctrl_pts[0][1] = vars_nctrl; // -v, was 128
        d_args.vars_nctrl_pts[0][2] = vars_nctrl; // -v, was 128 
        // d_args.ndom_pts[0]  = 64; // -n, was 256;
        // d_args.ndom_pts[1]  = 64; // -n, was 256;
        // d_args.ndom_pts[2]  = 64; // -n, was 256;
        // d_args.vars_nctrl_pts[0][0] = 32; // -v, was 128
        // d_args.vars_nctrl_pts[0][1] = 32; // -v, was 128
        // d_args.vars_nctrl_pts[0][2] = 32; // -v, was 128 

        d_args.infile = infile;
        if (dom_dim == 3)
            master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
                    { b->read_3d_scalar_data_multiblock(cp, d_args); });
        else
        {
            fprintf(stderr, "general data only available in 3d domain\n");
            exit(0);
        }
    }
#endif
